...
final class My_Slider_Plugin {
	...
	private function slide_editor( $index, $slide ) {
		$slide = wp_parse_args( $slide, array( 'type' => 'image', 'title' => '', 'content' => '', 'image' => '', 'image_id' => 0, 'image_alt' => '', 'video' => '', 'button_text' => '', 'button_url' => '', 'background' => '#1d2327', 'overlay_opacity' => 20, 'align' => 'left', 'hide_mobile' => 0 ) );
	...
	private static function defaults() {
		return array( 'autoplay' => 1, 'pause_hover' => 1, 'loop' => 1, 'arrows' => 1, 'dots' => 1, 'keyboard' => 1, 'wheel' => 0, 'auto_height' => 1, 'shadow' => 0, 'animation' => 'slide', 'duration' => 5000, 'speed' => 500, 'height' => '', 'layout' => 'full', 'max_width' => 1200, 'radius' => 0 );
	}
	...
	public function shortcode( $atts ) {
		...
		$style = sprintf( '--my-slider-speed:%dms;--my-slider-radius:%dpx;%s%s', absint( $settings['speed'] ), absint( $settings['radius'] ), empty( $settings['height'] ) ? '' : 'height:' . absint( $settings['height'] ) . 'px;', 'boxed' === $settings['layout'] ? 'max-width:' . absint( $settings['max_width'] ) . 'px;margin-inline:auto;' : '' );
		ob_start();
		?>
		<div class="my-slider my-slider--<?php echo esc_attr( $settings['animation'] ); ?><?php echo ! empty( $settings['shadow'] ) ? ' my-slider--shadow' : ''; ?>" style="<?php echo esc_attr( $style ); ?>" data-settings="<?php echo esc_attr( wp_json_encode( $settings ) ); ?>" tabindex="0" role="region" aria-roledescription="carousel" aria-label="<?php echo esc_attr( get_the_title( $id ) ); ?>">
	...
	private function render_slide( $slide ) {
		...
		if ( ! empty( $slide['button_text'] ) && ! empty( $slide['button_url'] ) ) echo '<a class="my-slider__button" href="' . esc_url( $slide['button_url'] ) . '">' . esc_html( $slide['button_text'] ) . '</a>';
		return ob_get_clean();
	}
