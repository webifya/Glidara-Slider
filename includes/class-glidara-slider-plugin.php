...
final class Glidara_Slider_Plugin {
	...
	public function admin_menu() {
		add_submenu_page( 'edit.php?post_type=glidara_slider', __( 'Go Pro', 'glidara-slider' ), __( 'Go Pro', 'glidara-slider' ), 'manage_options', 'glidara-slider-go-pro', array( $this, 'go_pro_page' ) );
	}
	...
	public function register_slider_type() {
		register_post_type(
			...
			array(
				'labels' => array(
					'name'          => __( 'Sliders', 'glidara-slider' ),
	...
	public function add_meta_boxes() {
		...
		add_meta_box( 'glidara-slider-settings', __( 'Slider Settings', 'glidara-slider' ), array( $this, 'settings_box' ), 'glidara_slider', 'side' );
	}
	...
	private function slide_editor( $index, $slide ) {
		...
		<div class="glidara-slider-slide">
			...
			<div class="glidara-slider-fields">
				...
				<label><?php esc_html_e( 'Heading', 'glidara-slider' ); ?><input type="text" name="<?php echo esc_attr( $name ); ?>[title]" value="<?php echo esc_attr( $slide['title'] ); ?>"></label>
				<label><?php esc_html_e( 'Content / HTML / Shortcode', 'glidara-slider' ); ?><textarea name="<?php echo esc_attr( $name ); ?>[content]" rows="4"><?php echo esc_textarea( $slide['content'] ); ?></textarea></label>
	...
	private function layer_editor( $slide_index, $layer_index, $layer ) {
		...
		<div class="glidara-slider-layer">
			...
			<label><?php esc_html_e( 'Type', 'glidara-slider' ); ?><select name="<?php echo esc_attr( $name ); ?>[type]"><?php foreach ( array( 'heading', 'text', 'button', 'image', 'icon' ) as $type ) : ?><option value="<?php echo esc_attr( $type ); ?>" <?php selected( $layer['type'], $type ); ?>><?php echo esc_html( ucfirst( $type ) ); ?></option><?php endforeach; ?></select></label>
	...
	public function settings_box( $post ) {
		...
		foreach ( array( 'autoplay' => 'Autoplay', 'pause_hover' => 'Pause on hover', 'loop' => 'Infinite loop', 'arrows' => 'Arrows', 'dots' => 'Dots', 'keyboard' => 'Keyboard navigation', 'wheel' => 'Mouse wheel', 'auto_height' => 'Auto height' ) as $key => $label ) {
			printf( '<p><label><input type="checkbox" name="glidara_slider_settings[%1$s]" value="1" %2$s> %3$s</label></p>', esc_attr( $key ), checked( ! empty( $settings[ $key ] ), true, false ), esc_html( $label ) );
		}
		?>
		<p><label><?php esc_html_e( 'Animation', 'glidara-slider' ); ?><select name="glidara_slider_settings[animation]">
		...
		<p><label><?php esc_html_e( 'Slide duration (ms)', 'glidara-slider' ); ?><input type="number" min="1000" step="100" name="glidara_slider_settings[duration]" value="<?php echo esc_attr( $settings['duration'] ); ?>"></label></p>
		...
		<hr><strong><?php esc_html_e( 'Global styles', 'glidara-slider' ); ?></strong>
		<p><label><?php esc_html_e( 'Accent color', 'glidara-slider' ); ?><input type="color" name="glidara_slider_settings[accent]" value="<?php echo esc_attr( $settings['accent'] ); ?>"></label></p>
	...
	private function render_layers( $layers ) {
		...
		foreach ( (array) $layers as $layer ) {
			...
			$classes = 'glidara-slider__layer glidara-slider__layer--' . sanitize_html_class( $type );
